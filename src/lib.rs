...
impl CanonicalBytes {
    /// Construct from already-canonicalized bytes. Crate-private: the only
    /// way to get a [`CanonicalBytes`] is to feed input through
    /// [`canonical_bytes_from_slice`] (or the re-export in
    ...
    /// True when the canonical output is empty (only possible for an empty
    /// input in degenerate paths; not reachable from the primary API).
    #[must_use]
    pub fn is_empty(&self) -> bool {
        self.0.is_empty()
    }

    /// Consume the wrapper and return the underlying byte buffer. Use at
    /// wire boundaries where ownership is transferred (file write, network
    /// send). Not an implicit coercion.
    #[must_use]
